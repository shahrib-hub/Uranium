    ...
    <div className="p-6 sm:p-10 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
        <div>
          ...
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black tracking-tighter uppercase italic text-white">
            {guild?.name ? (
              ...
              <>Server <span className="text-red-500">Overview</span></>
            )}
          </h1>
        ...
        <button
          ...
          onClick={refresh}
          disabled={refreshing}
          className="self-start sm:self-auto h-10 px-4 rounded-xl bg-[#1a1b26] hover:bg-[#222433] border border-[#2a2c3d] text-xs font-bold text-white/80 hover:text-white flex items-center gap-2 transition disabled:opacity-50 active:scale-95"
        >
      ...
      <div className="flex items-center gap-6 border-b border-[#232534] text-sm font-bold">
        {[
          ...
          >
            <span>{tab.label}</span>
            {activeTab === tab.id && (
              <span className="absolute bottom-0 inset-x-0 h-0.5 bg-red-500 rounded-full" />
            )}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <Metric
              ...
              label="Bot Ping"
              value={stats?.ping !== undefined && stats?.ping !== null ? `${String(stats.ping).replace(/ms/gi, '').trim()}ms` : '—'}
      ...
      {activeTab === 'stats' && (
        <div className="rounded-2xl border border-[#232534] bg-[#161722] p-6 space-y-4">
          ...
          <div className="divide-y divide-white/5 text-xs">
            ...
            <Row label="Gateway Ping" value={stats?.ping !== undefined && stats?.ping !== null ? `${String(stats.ping).replace(/ms/gi, '').trim()}ms` : '—'} />
      ...
      {/* Minimalist Tip Banner at Bottom */}
      <div className="rounded-xl border border-[#232534] bg-[#161722] p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <span className="px-2.5 py-1 rounded-md bg-white/10 text-xs font-black uppercase text-white shrink-0">
            Tip
          </span>
          <p className="text-sm text-white/70">
            Control moderation filters, lossless audio, and role assignment directly from Discord slash commands.
          </p>
        </div>
        <Link
          href={`/commands?guild=${guildId}`}
          className="h-9 px-4 rounded-xl bg-white !text-[#0e0f15] hover:bg-neutral-200 text-xs sm:text-sm font-black flex items-center gap-2 transition shrink-0 shadow-sm"
          style={{ color: '#0e0f15' }}
        >
          <span style={{ color: '#0e0f15', fontWeight: 800 }}>See all commands</span>
          <ArrowRight size={14} style={{ color: '#0e0f15' }} />
        </Link>
      </div>
    </div>
  );
  ...
  return (
    <article className="rounded-2xl border border-[#232534] bg-[#161722] p-5 space-y-3">
      ...
      <div>
        ...
        <p className="text-xs text-white/40 truncate mt-1">{detail}</p>
      </div>
    </article>
  );
}
